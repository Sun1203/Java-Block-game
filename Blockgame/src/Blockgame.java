...
public class Blockgame {
	...
	static class MyFrame extends JFrame {
		...
		static class Block {
			...
			boolean isHidden = false; // 충돌후 사라지게 한다. 
		}
		...
		public void initData() {
			for (int i=0;i<BLOCK_ROWS;i++) {
				for (int j=0;j<BLOCK_COLUMNS;j++) {
					...
					blocks[i][j].color = 4 - i; //  0 :white 1 : yellow 2:blue 3:mazanta 4:
		...
		public void setKeyListener() {
			this.addKeyListener( new KeyAdapter() {
				@Override
				public void keyPressed(KeyEvent e) { // Key Event : 컴퓨터에서 키보드값 입력받
					if (e.getKeyCode() == KeyEvent.VK_LEFT) {
						System.out.println("Pressed Left Key");
						barXTarget -= 20;
						if ( bar.x < barXTarget ) { // 계속 키를 눌렀을때. 
							barXTarget = bar.x;
						}
					}
		...
		public  boolean duplRect(Rectangle rect1, Rectangle rect2) {
			return rect1.intersects(rect2); // check two Rect is Duplicated! 서로 영역이 중복되는지 확인.
		}
		public void checkCollision() {
			if (dir == 0) { // 0 : Up-Right
				// Wall
				if (ball.y < 0) dir = 1; // wall upper
				if (ball.x > CANVAS_WIDTH - BALL_WIDTH) dir = 2; // wall right
				//Bar
				//none
				...
				if (ball.x > CANVAS_WIDTH - BALL_WIDTH) dir = 3; // wall left
				//Bar
				if ( ball.getBottomCenter().y >= bar.y ) {
					if ( duplRect (new Rectangle(ball.x, ball.y, ball.width, ball.height) , 
							new Rectangle (bar.x, bar.y, bar.width, bar.height))) dir = 0;
				}
			} else if (dir == 2) { // 2 : Up-Left
				// Wall
				if (ball.y < 0) dir = 3; // wall upper
				if (ball.x < 0) dir = 0; // wall left
				//Bar
				// none
			}else if (dir == 3) { // 3 : Down-Left
				if (ball.y > CANVAS_HEIGHT - BALL_HEIGHT - BALL_HEIGHT) {
				...
				if ( ball.getBottomCenter().y >= bar.y ) {
					if ( duplRect (new Rectangle(ball.x, ball.y, ball.width, ball.height) , 
							new Rectangle (bar.x, bar.y, bar.width, bar.height))) dir = 2;
				}
			}
					...
					if (block.isHidden == false) {
						...
						if (dir ==0) { //  Up-Right	
							if ( duplRect (new Rectangle(ball.x, ball.y, ball.width, ball.height) , 
								...
								else if (block.color == 2) score += 10;
								else if (block.color == 3) score += 10;
								else if (block.color == 4) score += 10;
							}
